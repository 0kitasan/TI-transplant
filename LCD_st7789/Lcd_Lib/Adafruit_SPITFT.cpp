...
//  callback(SpiCmd::transmit_byterev, (uint8_t*)colors, len);
//  return;
...
//  data[0] = color >> 8;
//  data[1] = color;
//  callback(SpiCmd::transmit_repeat, data, len);
//  return;
...
//void Adafruit_SPITFT::drawChar(int16_t x, int16_t y, unsigned char c,
...
//    startWrite();
//
//    GFXcanvas16 canvas(6 * size_x, 8 * size_y);
//    canvas.drawChar(0, 0, c, color, bg, size_x, size_y);
//    drawRGBBitmap(x, y, canvas.getBuffer(), 6 * size_x, 8 * size_y);
//
//    endWrite();
//
//  } else { // Custom font
